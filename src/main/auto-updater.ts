...
const REPO = 'naer-lily/asuka'
const API_LATEST = `https://api.github.com/repos/${REPO}/releases/latest`
const UPDATE_LOG = join(homedir(), '.asuka', 'logs', 'updater.log')
...
async function httpGetJson<T>(url: string): Promise<T> {
  return new Promise((resolve, reject) => {
    const req = get(url, { headers: { 'User-Agent': 'asuka-updater/1.0' } }, (res) => {
...
class AutoUpdaterManager {
  ...
  async checkForUpdates(): Promise<UpdateInfo> {
    if (this.updating) {
      mainLogger.warn('[Updater] check blocked: update already in progress')
      return { available: false, currentVersion: app.getVersion() }
    }

    const currentVersion = app.getVersion()
    mainLogger.info('[Updater] checking for updates, current=%s', currentVersion)
    try {
      ...
      mainLogger.info('[Updater] check complete available=%s latest=%s', available, latestVersion)
      return { available, currentVersion, latestVersion: this.cached.latestVersion }
    } catch (e) {
      mainLogger.error('[Updater] check failed:', e)
  ...
  async downloadAndInstall(onToast?: (msg: string) => void): Promise<void> {
    ...
    try {
      const tmpDir = join(tmpdir(), `asuka-update-${Date.now()}`)
      mkdirSync(tmpDir, { recursive: true })
      const zipPath = join(tmpDir, 'update.zip')
      const extractDir = join(tmpDir, 'extracted')

      mainLogger.info('[Updater] downloading %s', downloadUrl)
      onToast?.('正在下载更新...')

      await downloadFile(downloadUrl, zipPath)

      mainLogger.info('[Updater] extracting')
      onToast?.('正在安装，Asuka 即将重启...')
...
$srcDir = $NewDir
$found = Get-ChildItem -Path $NewDir -Recurse -Filter 'Asuka.exe' -ErrorAction SilentlyContinue | Select-Object -First 1
if ($found) {
  $srcDir = Split-Path $found.FullName -Parent
  Log "Found Asuka.exe at $($found.FullName), srcDir=$srcDir"
} else {
  Log "ERROR: Asuka.exe not found in $NewDir"
  exit 1
}
...
Log "Updater exiting"`
      ...
      mainLogger.info('[Updater] script written path=%s size=%d logDir=%s exists=%s', scriptPath, scriptSize, logDir, logDirOk)
      const spawnArgs = [
        '/c', 'start', '', '/min',
        'powershell',
      ...
      mainLogger.info('[Updater] spawn args: cmd %s', spawnArgs.map((a, i) => `${i}:${a}`).join(' '))
      mainLogger.info('[Updater] launching updater via cmd /c start, appDir=%s log=%s', appDir, UPDATE_LOG)

      const cp: ChildProcess = spawn('cmd', spawnArgs, { detached: true, stdio: 'ignore' })

      if (cp.pid) {
        mainLogger.info('[Updater] child spawned pid=%d', cp.pid)
      } else {
        mainLogger.error('[Updater] child spawned but pid is undefined!')
      }

      cp.on('error', (err) => {
        mainLogger.error('[Updater] spawn error:', err)
      })

      cp.unref()

      setTimeout(() => {
        mainLogger.info('[Updater] calling app.quit()')
        app.quit()
      }, 2000)
    } catch (e) {
      this.updating = false
